...
- getPrincipal - получение пользователя(UserDetails)
...
### Точки входа
Точка входа - это компонент интерфейса AuthenticationEntryPoint, которая позволяет отправить пользователя на аутентификацию. Когда пользователь пытается получить доступ к ресурсу для которого он должен быть аутентифицирован, то выбрасывается исключение и точка входа как раз обрабатывает это исключение и перенаправляет пользователя.![[Pasted image 20241030092056.png]]
Т.е. именно EntryPoint перенаправляет пользователя на страничку логина или в случае Basic ставит в header WWW-Authenticate.
commence:
- request - тот реквест, для выполнения которого нужна аутинтентификация
- response - тот ответ, который можем дать. Записать хедеры, например.
- authException - как раз так ошибка, которая произошла во время попытка аутентификации.
Вот так реализован EntryPoint для BasicAuthentication
![[Pasted image 20241030092319.png]]
Как видим, он ставит хедер и ставит 401 статус ответа.

Как нам задать свою точку входа? Для этого нужно в конфигурации SecurityFilterChain зайти в exceptionHandling и там вызвать authenticaionEntyPoint, куда и передать точку входа. Она будет глобальной для всех запросов. Т.е. любой запрос, который будет не аутентифицирован будет попадать в нашу точку входа. В нашем случае мы решили, что будет выводить stackTrace исключения и в респонсе отправлять 401 статус.
![[Pasted image 20241030092706.png]]

А теперь сделаем перенаправление на страницу аутентификации. Но в таком случае, нам нужно дать доступ к ней. Для этого в requestMatchers указываем, что все ресурсы по URL public будут доступны всем(permitAll)
![[Pasted image 20241030093344.png]]

 Причем точки входа есть как глобальные, так и связанные с конкретным фильтром. И в первый раз будет отрабатывать глобальная точка входа, а потом уже с фильтром.
 Например, вот здесь мы для httpbasic указали свой entryPoint - это как раз связанная с фильтром. Связанная с фильтром срабатывает тогда, когда пользователь ввел неправильные данные или по каким-то другим причинам аутентификация не прошла. А глобальная точка входа срабатывает, когда пользователь в первый раз обратился к какому-то ресурсу, а для его получения нужно аутентификация.
![[Pasted image 20241030093926.png]]
И таким образом, то, что мы написали выше отработает так: 
Пользователь попытается получить доступ к ресурсу, он не аутентифицирован, следовательно, сработает глобальная точка входа, которая заставит его ввести свои данные. После этого, если он неправильно ввел данные, то сработает уже точка входа фильтра, которая вернет пустую страницу, потому что как видим, лямбда ничего не делает, которую мы указали в httpBasic().aythenticationEntryPoint.

И мы можем добавлять точки входа для различных url. Т.е. указываем url и точку входа и тогда, когда по такому URL будет заходить пользователь и аутентифицироваться, то будет вызываться именно эта точка.

### Написание своих фильтров
1. Создать класс, который наследуется от OncePerRequestFilter(удобнее тем, что в нем в методе doFilter уже будут HttpServletRequest, а не просто ServletRequest как в дефолтном интерфейсе Filter). Реализовать в этом классе метод doFilter.
2. Добавить фильтр в filterChain. Для этого при конфигурации SecurityFilterChain нужно в HttpSecurity добавить фильтр, используя addFilterBefore или after для того, чтобы добавить наш фильтр перед какими-то уже зарегистрированным фильтром или после.

Если хотим написать свой фильтр аутентификации, то можно испоьзовать AuthenticationFilter.
